...
const fs = require('fs');
addFilter('statusTag_V10', function (statusID, statuses) {
    var statusName = null
...
addFilter('claimCount_V10', function (statusID, claims, claimType) {
    let i = 0
...
addFilter('pageCount_V10', function (content, perPage) {
    return Math.ceil(content / perPage)
})

addFilter('uniqueDates_V10', function (claims, dateType) {
...
addFilter('statusName_V10', function (statusID, statuses) {
    var statusName = null
    for (const s of statuses) {
        if (s.id == statusID) {
            statusName = s.name
...
addFilter('statusDetails_V10', function (statusID, statuses) {
    let status = null
    for (const s of statuses) {
        if (s.id == statusID) {
            status = s
...
addFilter('variableDate_V10', function (statusID) {
...
addFilter('removeSpacesAndLowerCase_V10', function (inputString) {
    // Convert the string to lowercase
    let outputString = removeSpacesAndLowerCase(inputString);
    return outputString;
})

addFilter('potName_V10', function (type) {
...
addFilter('checkEligible_V10', function (learner, type, roleTypes) {
    let eligibleRoles = []
    if (type == "TU") {
...
addFilter('groupByTitle_V10', function (training) {
...
addFilter('getUniqueCourseTitles_V10', function (training) {
    const qualificationsObject = training.find(obj => obj.groupTitle == "Qualifications");
    const uniqueTitles = [];
...
addFilter('formatCount_V10', function (courses) {
...
addFilter('dateErrorFormat_V10', function (dateErrorObject, type) {
    let state = false
    if (dateErrorObject) {
        if (type == "day") {
...
addFilter('listItemVariableDate_V10', function (statusID, claim) {
    if (statusID == 'not-yet-submitted') {
...
addFilter('listItemVariableSort_V10', function (statusID, claim) {
    if (statusID == 'not-yet-submitted') {
...
addFilter('relativeDateFromDateToToday_V10', function (dateStr) {
...
addFilter('findMatchingTraining_V10', function (claim, training) {
...
addFilter('formatTrainingDate_V10', function (date) {
...
addFilter('learnerErrorMessage_V10', function (submitError) {
    let errorSummaryStr = ''
...
addFilter('learnerMatch_V10', function (newField, matchField, type) {
    let result = ''
...
addFilter('learnerSearch_V10', function (search, learner) {
    let match = false
...
addFilter('trainingSearch_V10', function (search, training) {
    let match = false
    const formattedSearch = removeSpacesAndLowerCase(search);
...
addFilter('bankErrorMessage_V10', function (bankErrorObject) {
    const errorMessages = [];
...
addFilter('trainingTypeCheck_V10', function (trainingCode, trainingList, matchType) {
...
addFilter('findPair_V10', function (claimID, claims) {
