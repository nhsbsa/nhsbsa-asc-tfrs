...
const { checkClaim, compareNINumbers, sortByCreatedDate, generateUniqueID, validateDate, checkDuplicateClaim, checkLearnerForm, checkBankDetailsForm, loadJSONFromFile, checkUserForm } = require('../_helpers/helpers.js');
...
router.post('/search_id_result', function (req, res) {
  ...
  if (!emptyRegex.test(claimID)) {
    return res.redirect('claims/v13/manage-claims-home?fundingPot=TU&emptyError=true');
  }

  const letterORegex = /o/i;
  if (letterORegex.test(claimID)) {
    return res.redirect('claims/v13/manage-claims-home?fundingPot=TU&invalidIDError=true');
  }

  const lengthRegex = /^[A-NP-Z0-9]{3}(-)?[A-NP-Z0-9]{4}(-)?[A-NP-Z0-9]{4}(-)?([ABC])?$/;
  if (!lengthRegex.test(claimID)) {
    return res.redirect('claims/v13/manage-claims-home?fundingPot=TU&searchId='+claimID + '&invalidIDError=true');
  }
  ...
  if (foundClaim == null) {
    return res.redirect('claims/v13/manage-claims-home?fundingPot=TU&searchId='+claimID + '&notFound=true');
  } else {
    res.redirect('claims/v13/claim/claim-details?id=' + claimID +"&fromSearchId=true");
  }
});
...
router.post('/search_result_a', function (req, res) {
  ...
  if (errorQuery == "") {
    res.redirect('claims/v13/claim/search-version-a?fromSearchResults=true#searchResults');
  } else {
    res.redirect('claims/v13/claim/search-version-a?' + errorQuery)
  }
});
...
router.post('/search_result_b', function (req, res) {
  ...
  if (training == "" && learner == "" && submitter == "" && statusArray == null && typeArray == null && startMonth == "" && startYear == "" && endMonth == "" && endYear == "") {
    return res.redirect('claims/v13/claim/search-version-b?noInputsB=true');
  } 
  if (startMonth != "" || startYear != "" || endMonth != "" || endYear != "") {
    if (isNaN(startMonth) || isNaN(startYear) || isNaN(endMonth) || isNaN(endYear)) {
      return res.redirect('claims/v13/claim/search-version-b?dateInvalid=notValidDates');
    } else if ((startMonth < 1 || startMonth > 12) || (endMonth < 1 || endMonth > 12) || (startYear < 1 || startYear > 2024) || (endYear < 1 || endYear > 2024)) {
      return res.redirect('claims/v13/claim/search-version-b?dateInvalid=notValidDates');
    } else if (startDate > endDate) {
      return res.redirect('claims/v13/claim/search-version-b?dateInvalid=startBeforeEnd');
    } else if (startMonth == "" || startYear == "" || endMonth == "" || endYear == "") {
      return res.redirect('claims/v13/claim/search-version-b?dateInvalid=notValidDates');
    } else {
      return res.redirect('claims/v13/claim/search-version-b?fromSearchResults=true#searchResults');
    }
  } else {
    return res.redirect('claims/v13/claim/search-version-b?fromSearchResults=true#searchResults');
  }
...
router.get('/new-claim-v13', function (req, res) {
  ...
  delete req.session.data['invalidIDError'];
  delete req.session.data['notFound'];
  res.redirect('claims/v13/claim/select-training')
});
...
function loadData(req) {
  ...
  const path = 'app/views/claims/v13/_data/'
