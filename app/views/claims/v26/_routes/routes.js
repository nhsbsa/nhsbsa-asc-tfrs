...
router.use('/claims/v26/backstop', require('../_backstop/backstop-routes.js'));

// v26 Prototype routes
...
router.post('/detailsCorrectResponse', function (req, res) {
  ...
  if (action =="continue") {
    ...
    req.session.data.banner.regRef
...
router.post('/validate-org-name', function (req, res) {
  delete req.session.data.orgNameEmptyError
...
router.post('/check-answer-confirmation', function (req, res) {
  const action = req.session.data.action
...
router.get('/showPaymentNote', function (req, res) {
  ...
  for (const c of req.session.data.claims ) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.org.workplaceID)) {
      res.redirect('claims/v26/claim/previousSubmissionsTable?subCount=' + subCount + '&id=' + claimID + "#claimDetails")
    }
  }
});
...
router.get('/hidePaymentNote', function (req, res) {
  ...
  for (const c of req.session.data.claims) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.org.workplaceID)) {
      res.redirect('claims/v26/claim/previousSubmissionsTable' + '?id=' + claimID + "#claimDetails")
    }
  }
});
...
router.get('/showLearnerNote', function (req, res) {
  ...
  for (const c of req.session.data.claims ) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.org.workplaceID)) {
      res.redirect('claims/v26/claim/learnerSubmissionsTable?subCount=' + subCount + '&id=' + claimID + "#" + req.session.data.slotID)
    }
  }
});
...
router.get('/hideLearnerNote', function (req, res) {
  ...
  res.redirect('claims/v26/claim/learnerSubmissionsTable' + '?id=' + claimID + "#" + req.session.data.slotID)
});
...
router.get('/applySubmissionsSort', function (req, res) {
  var claimID = req.session.data.id
  res.redirect('claims/v26/claim/previousSubmissionsTable' + '?id=' + claimID + "&filter=" + req.session.data.filter)
});

router.get('/applyLearnerSort', function (req, res) {
  var claimID = req.session.data.id
  res.redirect('claims/v26/claim/learnerSubmissionsTable' + '?id=' + claimID + "&filter=" + req.session.data.filter)
});
...
router.post('/generate-handler', function (req, res) {
  const jsonFilePath = './app/views/claims/v26/_data/claims.json';
