...
const { checkClaim, compareNINumbers, sortByCreatedDate, generateUniqueID, validateDate, checkDuplicateClaim, checkLearnerForm, checkBankDetailsForm, loadJSONFromFile, checkUserForm, getLearner } = require('../_helpers/helpers.js');
...
router.get('/new-claim', function (req, res) {
  if (req.session.data.fundingPot == "TU") {
    res.redirect('claims/v11/claim/select-training')
  } else if (req.session.data.fundingPot == "CPD") {
    const claimID = newCPDClaim(req)
    res.redirect('claims/v11/claim/claim-details' + '?id=' + claimID )
  }
});
