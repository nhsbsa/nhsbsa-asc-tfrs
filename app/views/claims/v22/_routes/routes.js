...
router.use('/claims/v22/backstop', require('../_backstop/backstop-routes.js'));

// v22 Prototype routes
...
router.get('/showPaymentNote', function (req, res) {
  ...
  for (const c of req.session.data.claims ) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.org.workplaceID)) {
      res.redirect('claims/v22/claim/previousSubmissionsTable?subCount=' + subCount + '&id=' + claimID + "#claimDetails")
    }
  }
});
...
router.get('/hidePaymentNote', function (req, res) {
  ...
  for (const c of req.session.data.claims) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.org.workplaceID)) {
      res.redirect('claims/v22/claim/previousSubmissionsTable' + '?id=' + claimID + "#claimDetails")
    }
  }
});
...
router.get('/showLearnerNote', function (req, res) {
  ...
  for (const c of req.session.data.claims ) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.org.workplaceID)) {
      res.redirect('claims/v22/claim/learnerSubmissionsTable?subCount=' + subCount + '&id=' + claimID + "#" + req.session.data.slotID)
    }
  }
});
...
router.get('/hideLearnerNote', function (req, res) {
  ...
  res.redirect('claims/v22/claim/learnerSubmissionsTable' + '?id=' + claimID + "#" + req.session.data.slotID)
});
...
router.get('/applySubmissionsSort', function (req, res) {
  var claimID = req.session.data.id
  res.redirect('claims/v22/claim/previousSubmissionsTable' + '?id=' + claimID + "&filter=" + req.session.data.filter)
});

router.get('/applyLearnerSort', function (req, res) {
  var claimID = req.session.data.id
  res.redirect('claims/v22/claim/learnerSubmissionsTable' + '?id=' + claimID + "&filter=" + req.session.data.filter)
});
...
router.post('/generate-handler', function (req, res) {
  const jsonFilePath = './app/views/claims/v22/_data/claims.json';
