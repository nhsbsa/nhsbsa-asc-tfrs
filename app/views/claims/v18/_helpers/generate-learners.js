...
function generateLearners (quantity) {
  ...
  const jobTitles = JSON.parse(fs.readFileSync('./app/views/claims/v18/_data/jobTitles.json', 'utf8'));

  //load pre set learners from JSOn file
  const preSetLearners = JSON.parse(fs.readFileSync('./app/views/claims/v18/_data/pre-set-learners.json', 'utf8'));
  ...
  const jsonFilePath = './app/views/claims/v18/_data/learners.json';
