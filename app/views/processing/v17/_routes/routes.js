...
router.get('/showClaimPaymentNote', function (req, res) {
  ...
  for (const c of req.session.data.claims ) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.orgID)) {
      res.redirect('processing/v17/organisation/org-view-main?subCount=' + subCount + '&orgTab=singleClaim' + '&id=' + claimID)
    }
  }
});
...
router.get('/hideClaimPaymentNote', function (req, res) {
  ...
  for (const c of req.session.data.claims) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.orgID)) {
      res.redirect('processing/v17/organisation/org-view-main?orgTab=singleClaim' + '&id=' + claimID)
    }
  }
});
...
router.get('/applySubmissionsFilterProcessorv17', function (req, res) {
  var claimID = req.session.data.id
  var filter = req.session.data.filter
  res.redirect('processing/v17/organisation/org-view-main?orgTab=singleClaim' + '&id=' + claimID + "&filter=" + filter)
})
...
router.post('/confirm-details-outcome', function (req, res) {
  ...
  if (verificationResponse == "fullMatch") {
    ...
    res.redirect('/processing/v17/organisation/org-view-main?orgTab=bank-details&bankDetailsVerified=true')
    ...
    res.redirect('/processing/v17/organisation/org-view-main?orgTab=bank-details&bankDetailsRejected=true')
  }
...
router.post('/generate-handler', function (req, res) {
  const jsonFilePath = './app/views/processing/v17/_data/claims.json';
