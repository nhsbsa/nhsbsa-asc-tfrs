...
function generateClaim(claimTypeInput, claimStatusInput, submissionsInput, learnersInput, compDate, matchingClaim) {
  const learnerList = JSON.parse(fs.readFileSync('./app/views/processing/v15/_data/learners.json', 'utf8'));
  const training = JSON.parse(fs.readFileSync('./app/views/processing/v15/_data/training.json', 'utf8'));
  const claims = JSON.parse(fs.readFileSync('./app/views/processing/v15/_data/claims.json', 'utf8'));
  const organisations = JSON.parse(fs.readFileSync('./app/views/processing/v15/_data/organisations.json', 'utf8'));
...
function generateSubmission(submissions, claimType, isPaymentPlan, claimStatusInput, compDate, trainingCode, claimLearners, createdDate, policyDate, users) {
  const backOfficeStaff = JSON.parse(fs.readFileSync('./app/views/processing/v15/_data/backOfficeStaff.json', 'utf8'));
  const learnerList = JSON.parse(fs.readFileSync('./app/views/processing/v15/_data/learners.json', 'utf8'));
      ...
      if (submissions.length === 0 ) {
        ...
        if (claimType == "40" || claimType == "100") {
          ...
          for (const learner of submission.learners) {
            if (submission.sharedCompletionDate || submission.learners.length <= 1) {
