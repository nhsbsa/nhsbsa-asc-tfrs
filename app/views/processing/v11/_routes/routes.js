...
router.use('/processing/v11/backstop', require('../_backstop/backstop-routes.js'));
...
router.get('/showEditedNote', function (req, res) {
  ...
  for (const c of req.session.data.claims ) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.orgID)) {
      res.redirect('processing/v11/organisation/org-view-main?subCount=' + subCount + '&orgTab=singleClaim' + '&id=' + claimID)
    }
  }
});
...
router.get('/hideEditedNote', function (req, res) {
  ...
  for (const c of req.session.data.claims) {
    if (claimID.replace(/[-\s]+/g, '') == c.claimID.replace(/[-\s]+/g, '') && (c.workplaceID == req.session.data.orgID)) {
      res.redirect('processing/v11/organisation/org-view-main?orgTab=singleClaim' + '&id=' + claimID)
    }
  }
});
